...
module KoTournamentTestHelper
  ...
  def create_ko_tournament_with_seedings(player_count, tournament_attrs = {})
    ...
    # Generate unique IDs
    tournament_id = TEST_ID_BASE + rand(1000..9999)
  ...
  def cleanup_ko_tournament(data)
    ...
    tournament.games.destroy_all if tournament.games.any?
    tournament.seedings.destroy_all if tournament.seedings.any?
    tournament.tournament_monitor&.destroy
    tournament.destroy
    
    # Clean up players
    data[:players]&.each(&:destroy)
  end
  ...
  def assert_first_round_has_players(tournament)
    # Determine first round prefix based on player count
    player_count = tournament.seedings.count
    first_round_prefix = case player_count
                         when 0..8 then "qf"
                         when 9..16 then "16f"
                         when 17..24 then "32f"
                         when 25..32 then "16f"
    ...
    first_round = tournament.games.where("gname LIKE '#{first_round_prefix}%'")
    
    first_round.each do |game|
  ...
  def assert_player_reference_resolves(tournament_monitor, reference, expected_player_id)
    actual_player_id = tournament_monitor.ko_ranking(reference)
