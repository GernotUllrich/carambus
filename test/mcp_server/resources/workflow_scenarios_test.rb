...
class McpServer::Resources::WorkflowScenariosTest < ActiveSupport::TestCase
  ...
  # Plan 10-07 Drift-Guard-Update: 6 → 8 Scenarios (2 neue JSON-Spickzettel: sportwart-tagesablauf-vor-turnier + akkreditierung-am-turniertag).
  # mime_type ist jetzt variabel pro Slug; .read gibt Hash {content:, mime_type:} zurück.
  EXPECTED_SCENARIO_COUNT = 8

  test "all gibt 8 MCP::Resource-Instanzen zurück" do
  ...
  # Plan 10-07 Drift-Guard-Update: 3 → 5 JSON-Spickzettel (2 neue Slugs: sportwart-tagesablauf-vor-turnier + akkreditierung-am-turniertag).
  test "alle JSON-Spickzettel haben mime_type application/json" do
    ...
    assert_equal 5, json_resources.size
    ...
    assert_includes json_resources.map(&:uri), "cc://workflow/scenarios/sportwart-tagesablauf-vor-turnier"
    assert_includes json_resources.map(&:uri), "cc://workflow/scenarios/akkreditierung-am-turniertag"
  end
end
