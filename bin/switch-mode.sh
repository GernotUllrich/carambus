...
EOF
    ...
    log "Current mode: LOCAL (carambus_api_url is set, local database, local deployment server)"
}
...
default:
  ...
  club_id: 357
development:
  carambus_api_url:  # Empty for API mode (no external API calls)
  location_id: 1
  application_name: Carambus
  support_email: gernot.ullrich@gmx.de
  business_name: Ullrich IT Consulting
...
EOF
    ...
    log "Current mode: API (carambus_api_url is empty, API database, production deployment server)"
}
...
show_current_mode() {
    ...
    if [ -f "$PROJECT_ROOT/config/carambus.yml" ]; then
        local api_url=$(grep -A 1 "development:" "$PROJECT_ROOT/config/carambus.yml" | grep "carambus_api_url:" | head -1 | sed 's/.*carambus_api_url: *//' | sed 's/#.*$//' | xargs)
...
show_help() {
    ...
    echo "  LOCAL:  carambus_api_url is set, local database, local deployment server"
    echo "  API:    carambus_api_url is empty, API database, production deployment server"
}
