module TableMonitorsHelper
  def ranking_table(hash, opts = {})
    ...
    lines = rankings.map do |player, results|
      "<tr>" + "#{"<td>#{results["rank"]}</td>" if opts[:order].include?(:rank)}" + "<td>#{Player[player].lastname} #{Player[player].firstname[0]}.</td><td>#{results["points"]}</td><td>#{results["result"]}</td><td>#{results["innings"]}</td><td>#{results["hs"]}</td><td>#{results["gd"]}</td><td>#{results["bed"]}</td>
</tr>"
    end.join("\n")
    return ("
