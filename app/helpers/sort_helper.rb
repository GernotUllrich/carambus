module SortHelper
  def sortable(_relation, column, title, request_params, options = {})
    matching_column = column.to_s == request_params[:sort]
    direction = if matching_column
                  request_params[:direction] == "asc" ? "desc" : "asc"
                else # Ascending by default if switching columns
                  "asc"
                end

    custom_link_to request_params.merge(sort: column, direction: direction), options do
      concat title
