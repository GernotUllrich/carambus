module NavHelper
  def nav_link_to(name = nil, options = {}, html_options = {}, &block)
    ...
    return custom_link_to url, html_options, &block if block

    custom_link_to name, url, html_options
  end
  ...
  def header_with_anchor(title, header_tag: :h2, id: nil, icon: nil, header_class: "group",
                         link_class: "hidden align-middle group-hover:inline-block p-1", icon_class: "text-gray-500 h-4 w-4")
    ...
    tag.send(header_tag, id: id, class: header_class) do
      tag.span(title) + custom_link_to(icon, "##{id}", class: link_class)
    end
  end
