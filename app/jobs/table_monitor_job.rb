class TableMonitorJob < ApplicationJob
  ...
  def perform(*args)
    ...
    # Plan 40-01: Der Broadcast rendert ueber `ApplicationController.render` — also OHNE Request
    # und damit ohne `before_action :set_locale`. Bis hierher kam die Sprache aus `I18n.locale`
    # des Job-Threads: `default_locale`, oder — weil `I18n.locale` thread-lokal ist und
    # Job-Threads wiederverwendet werden — der Restwert eines fremden Requests. Ein per
    # `?locale=en` aufgerufenes Scoreboard fiel damit beim ersten Live-Update zurueck.
    #
    # `with_locale` statt Zuweisung: es stellt den vorigen Wert garantiert wieder her. Eine
    # Zuweisung wuerde in den wiederverwendeten Thread leaken und den naechsten Job faerben —
    # genau die Nicht-Determiniertheit, die hier behoben wird.
    #
    # Der GANZE case-Block ist umschlossen, nicht die fuenf Render-Aufrufe einzeln: so kann
    # keine Stelle vergessen werden, auch keine kuenftige.
    #
