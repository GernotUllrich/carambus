...
module ScrapeMonitor
  ...
  class_methods do
    ...
    def track_scraping(operation_name, &block)
      monitor = ScrapingMonitor.new(operation_name, name)
      monitor.run(&block)
    end
  end
...
class ScrapingMonitor
  ...
  def track_database_changes(&block)
    ...
    subscriber = ActiveSupport::Notifications.subscribe("sql.active_record") do |_name, _start, _finish, _id, payload|
      ...
      created_count += 1 if /^INSERT INTO/.match?(sql)
      updated_count += 1 if /^UPDATE/.match?(sql)
      deleted_count += 1 if /^DELETE FROM/.match?(sql)
    end
