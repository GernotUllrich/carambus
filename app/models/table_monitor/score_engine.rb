...
class TableMonitor::ScoreEngine
    ...
    if data["biathlon_phase"] != "3b" || n_balls <= to_play_3b
      ...
      if should_process_input
        if data["biathlon_phase"] == "3b"
          ...
          Rails.logger.debug { "add_n_balls: Processing input (n_balls=#{n_balls}, to_play=#{to_play}, allow_overflow=#{data["allow_overflow"].inspect})" }
          add = if data["allow_overflow"].present?
  ...
  # Returns nil otherwise.
  def set_n_balls(n_balls, change_to_pointer_mode = false) # rubocop:disable Lint/UnusedMethodArgument
  ...
  # Record a 2-point foul. Decrements by 2 and terminates the inning.
...
<td><span class=\"sm:text-xs lg:text-lg sm:px-2 lg:px-4\">#{
  ...
  # Accepts optional gps array for player name resolution (AR objects — caller
  ...
  def innings_history(gps: [])
    ...
    if data["free_game_form"] == "snooker"
      ...
      (0...num_rows).each do |i|
        break_balls_a << (i < break_balls_list_a.length ? break_balls_list_a[i] : (i == break_balls_list_a.length && active_player == "playera" ? break_balls_redo_a : nil))
        break_balls_b << (i < break_balls_list_b.length ? break_balls_list_b[i] : (i == break_balls_list_b.length && active_player == "playerb" ? break_balls_redo_b : nil))
