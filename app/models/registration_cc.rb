...
class RegistrationCc < ApplicationRecord
  ...
  def self.create_from_ba(tournament, opts)
    ...
    cat_scope = "Unisex"
    cat_scope = "Damen" if tournament.title =~ /Damen/
    cat_scope = "Herren" if tournament.title =~ /Herren/
    cat_scope = "Junioren" if tournament.title =~ /Junioren/
    cat_scope = "Junioren" if tournament.title =~ /Jugend/
    ...
    unless found
      ...
      doc
      _, doc = region_cc.post_cc('createMeldelisteSave', args, opts)
    end
    args = {
      fedId: region.region_cc.cc_id,
      branchId: branch_cc_id,
