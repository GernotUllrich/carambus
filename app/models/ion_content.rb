...
class IonContent < ApplicationRecord
  ...
  def self.scrape_images
    IonModule.where(module_type: ["textWithImage", "imageSubtitle", "gallery"]).each do |mod|
      doc = Nokogiri::HTML(mod.html)
      images = []
      doc.css("img").each do |img|
        url_img = img["src"]
        if url_img.present?
          image_name_matcher = url_img.match(/\/((?:thumb\/)?\d+\.(?:jpg|jpeg|png))/i)
          image_name_matcher = url_img.match(/\/(\d+\.(?:jpg|jpeg|png))/i)
          if image_name_matcher.present?
            ...
            unless File.exist?(image_file_name)
  ...
  def self.scrape_downloads
    IonModule.where(module_type: ["downloadDocument"]).each do |mod|
      ...
      unless File.exist?(dl_file_name)
  ...
  def self.scrape_website
    ...
    if res.message == 'OK'
      @ion_content = IonContent.find_or_create_by(page_id: 919568183)
      args = {
        "html" => doc.to_html,
...
=end
  ...
  def self.get_ioc(url, get_options = {}, opts = {})
