class RegionCcAction
  ...
  def self.get_base_opts_from_environment
    ...
    exclude_season_names = ["2009/2010", "2019/2020", "2020/2021", "2021/2022"]
    ...
    pool_ba_ids = League.joins(:discipline).where(organizer_id:1, organizer_type: "Region").where(disciplines: {name: "Pool"}).map(&:ba_id)
    ...
    snooker_ba_ids = League.joins(:discipline).where(organizer_id:1, organizer_type: "Region").where(disciplines: {name: "Snooker"}).map(&:ba_id)
    exclude_league_ba_ids = pool_ba_ids + snooker_ba_ids
