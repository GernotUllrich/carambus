...
class TournamentCc < ApplicationRecord
  ...
  belongs_to :group_cc
  belongs_to :championship_type_cc
  ...
  def self.create_from_ba(tournament, opts)
    ...
    tournament_cc = TournamentCc.where(name: tournament.title, discipline_id: tournament.discipline_id, branch_cc_id: branch_cc.id, season: opts[:season_name]).first
    begin
      args = {
    ...
    _, doc = region_cc.post_cc('createMeisterschaftSave', args, opts)
  ...
  def delete_tournament_results(opts)
    ...
    doc.css(".cc_bluelink").each do |line|
      ...
      _, doc2 = region.region_cc.post_cc_with_formdata('deleteErgebnis', args.merge(partieId: partieId), opts)
      doc2
    end
  ...
  def upload_csv(opts)
    ...
    tournament.games.where(game_scope).each do |game|
      ...
      if m = game.gname.match(/.*Runde/)
        game.gname = "Gruppe 1"
      end
      gruppe = GroupCc::NAME_MAPPING[:groups]["#{game.gname =~ /^group/ ? 'Gruppe' : game.gname}#{" #{game.group_no}" if game.group_no.present?}"] ||
        GroupCc::NAME_MAPPING[:round]["#{game.gname =~ /^group/ ? 'Gruppe' : game.gname}#{" #{game.group_no}" if game.group_no.present?}"]
      begin
      ...
      line = "#{gruppe};#{partie};;#{gp1.player.cc_id};#{gp2.player.cc_id};#{gp1.result};#{gp2.result};#{gp1.innings};#{gp2.innings};#{gp1.hs};#{gp2.hs}" rescue nil
    ...
    begin
      ranking_data = tournament.seedings.where(seeding_scope).select { |seeding| seeding.data["result"].andand["Gesamtrangliste"].present? }.sort_by { |seeding| seeding.data["result"]["Gesamtrangliste"]["Rank"].to_i }.map { |s| [s.data["result"]["Gesamtrangliste"]["#"].to_i, s.data["result"]["Gesamtrangliste"]["Punkte"].to_i, "", s.player.cc_id, "", ""].join(";") }
    rescue Exception => e
    ...
    f.close
    _, doc0a = region.region_cc.post_cc_with_formdata('importRangliste2', args.merge(importBut: "", ranglistenimport: UploadIO.new("#{Rails.root}/tmp/ranking#{tournament_cc.cc_id}.csv", "text/csv", "ranking#{tournament_cc.cc_id}.csv")), opts)
    ...
    f.close
    _, doc2 = region.region_cc.post_cc_with_formdata('importErgebnisseStep2', args.merge(disciplinId: tournament.discipline.discipline_cc.cc_id, saveBut: "", importFile: UploadIO.new("#{Rails.root}/tmp/result#{tournament_cc.cc_id}.csv", "text/csv", "result#{tournament_cc.cc_id}.csv")), opts)
