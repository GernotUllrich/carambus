...
class TableMonitor::GameSetup < ApplicationService
  ...
  def self.initialize_game(table_monitor:)
    ...
    #
    # 2026-04-28 Round-3 hardening: explizites data_will_change! + self.data = sichert
    # AR-Dirty-Tracking auch wenn der vorige deep_merge_data!-Aufruf (aus unbekanntem
    # Grund) data_will_change! nicht mehr aktiv hat. except() (nicht except!) liefert
    # eine neue Hash-Instanz ohne In-Place-Mutation.
    tm.data_will_change!
  ...
  def build_result_hash
...

    {
      ...
      # Bk2::AdvanceMatchState#derive_balls_goal via tournament_monitor.balls_goal
      # (Phase 38.4 D-06) with fallback to bk2_options["set_target_points"].
      # Without this whitelist entry the controller's p[:bk2_options] is silently dropped.
