...
class ProtocolPdf
  ...
  def draw_protocol_sheet(pdf)
    ...
    pdf.bounding_box([2, pdf.bounds.height - 2], width: pdf.bounds.width - 4, height: 40) do
      pdf.text sanitize_text(@location.name), size: 10, style: :bold, align: :center
      pdf.text sanitize_text(@location.address), size: 8, align: :center
    end

    # Horizontal line after address
    pdf.stroke_horizontal_line 0, pdf.bounds.width, at: pdf.bounds.height - 40

    # Game info line (Disziplin | Partie-Laenge | Partie-Nr. | Datum)
    pdf.bounding_box([2, pdf.bounds.height - 44], width: pdf.bounds.width - 4, height: 20) do
      ...
      pdf.text "#{discipline} | Partie-Laenge #{goal} | #{partie_nr} | #{date}", size: 10, align: :center
    end

    # Horizontal line after game info
    pdf.stroke_horizontal_line 0, pdf.bounds.width, at: pdf.bounds.height - 56
    ...
    pdf.bounding_box([2, pdf.bounds.height - 55], width: pdf.bounds.width - 4, height: 15) do
    ...
    pdf.stroke_horizontal_line 0, pdf.bounds.width, at: pdf.bounds.height - 72

    # Table section (13.5cm high = 383 points) with 26 rows
    table_y = pdf.bounds.height - 72
  ...
  def draw_protocol_table(pdf, start_y)
    ...
    balls_a = @player_a[:result] || ''
    balls_b = @player_b[:result] || ''
    innings_a = @player_a[:innings_count] || ''
    innings_b = @player_b[:innings_count] || ''
    gd_a = format("%.2f", balls_a.to_f / innings_a) || ''
    gd_b = format("%.2f", balls_b.to_f / innings_b) || ''
    hs_a = @player_a[:innings].map(&:to_i).max || ''
    hs_b = @player_b[:innings].map(&:to_i).max || ''
    ...
    table_data << [
      ...
      { content: "Summe", size: 4, font_style: :bold },
      { content: "Serie", size: 4, font_style: :bold },
      { content: "Summe", size: 4, font_style: :bold },
      { content: "Aufn.", size: 4, font_style: :bold, colspan: 2 },
      { content: "Serie", size: 4, font_style: :bold },
      { content: "Summe", size: 4, font_style: :bold },
      { content: "Serie", size: 4, font_style: :bold },
      { content: "Summe", size: 4, font_style: :bold },
    ...
    25.times do |i|
      ...
      serie_a_left = dash_if_zero(@player_a[:innings][inning_left]) || ''
      gesamt_a_left = dash_if_zero(@player_a[:totals][inning_left]) || ''
      serie_b_left = dash_if_zero(@player_b[:innings][inning_left]) || ''
      gesamt_b_left = dash_if_zero(@player_b[:totals][inning_left]) || ''

      serie_a_right = dash_if_zero(@player_a[:innings][inning_right]) || ''
      gesamt_a_right = dash_if_zero(@player_a[:totals][inning_right]) || ''
      serie_b_right = dash_if_zero(@player_b[:innings][inning_right]) || ''
      gesamt_b_right = dash_if_zero(@player_b[:totals][inning_right]) || ''
    ...
    table_data << [
      { content: balls_a.to_s, size: 10, colspan: 2},
      { content: "Anzahl der Bälle", size: 10, colspan: 6},

      { content: balls_b.to_s, size: 10, colspan: 2},
    ]
    table_data << [
      { content: innings_a.to_s, size: 10, colspan: 2},
      { content: "Anzahl der Aufnahmen", size: 10, colspan: 6},

      { content: innings_b.to_s, size: 10, colspan: 2},
    ]
    table_data << [
      { content: gd_a.to_s, size: 10, colspan: 2},
      { content: "Durchschnitt", size: 10, colspan: 6},

      { content: gd_b.to_s, size: 10, colspan: 2},
    ]
    table_data << [
      { content: hs_a.to_s, size: 10, colspan: 2},
      { content: "Höchstserie", size: 10, colspan: 6},

      { content: hs_b.to_s, size: 10, colspan: 2},
    ]
    ...
    pdf.bounding_box([1, start_y], width: pdf.bounds.width - 2, height: table_height) do
      pdf.table(table_data,
                ...
                cell_style: {
                  size: 10,
                  padding: [2, 1],  # Increased vertical padding to fill space
        ...
        (0...26).each do |i|
          t.row(i).height = table_height / 31
        end
      end
    end
  end
  ...
  def draw_footer(pdf)
    ...
    pdf.bounding_box([2, footer_y], width: pdf.bounds.width - 4, height: (2 * 28.35) - 2) do
      ...
      pdf.text_box "Anzahl der Baelle", at: [pdf.bounds.width - 120, y_pos], size: 8, align: :right, width: 95
      y_pos -= line_height
      pdf.text_box "Anzahl der Aufnahmen", at: [pdf.bounds.width - 120, y_pos], size: 8, align: :right, width: 95
      y_pos -= line_height
      pdf.text_box "Durchschnitt", at: [pdf.bounds.width - 120, y_pos], size: 8, align: :right, width: 95
      y_pos -= line_height
      pdf.text_box "Hoechstserie", at: [pdf.bounds.width - 120, y_pos], size: 8, align: :right, width: 95
    end
  end
