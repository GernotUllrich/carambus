...
# Kapselt die ClubCloud-Scraping-Logik für einzelne Ligen aus dem League-Modell.
# Verantwortlichkeiten:
#   - Eine einzelne League von ClubCloud scrapen (Mannschaften, Partien, Spielplan)
#   - Mannschaften, Spieler, Partien und PartyGames anlegen/aktualisieren
#   - GamePlan aus den gescrapten Daten erzeugen
#
# Verwendung:
#   League::ClubCloudScraper.call(league: league, league_details: true)
#
# ApplicationService gemäß D-07 des Extraktionsplans.
class League::ClubCloudScraper < ApplicationService
  ...
  def scrape_league
    ...
    if @opts[:cleanup]
      ...
      @league.parties.map { |p| p.party_games.send(destroy_method) }
      @league.parties.send(destroy_method)
      @league.league_teams.send(destroy_method)
    end
  ...
  def scrape_from_club_cloud
    ...
    league_p = "sb_spielplan.php?p=#{organizer.cc_id}--#{season.name}-#{@league.cc_id}#{"-#{@league.cc_id2}" if @league.cc_id2.present?}"
    ...
    parse_teams(league_doc, url)
    parse_parties(league_doc, url)
    build_game_plan(@disciplines)

    if @game_plan_data && @game_plan_data[:rows].present?
      save_game_plan
    end
  end
  ...
  def parse_teams(league_doc, url)
    ...
    team_table.css("tr").each do |tr|
      ...
      if /billard-union.net/.match?(url)
      ...
      if players_table.present?
        players_table.css("tr").each do |tr_p|
          ...
          if player.blank?
            if @league.organizer.shortname == "DBU"
              player = Player.find_by_dbu_nr(player_dbu_nr)
              if player.blank?
                player = Player.find_by_fl_name_and_cc_id(fl_name, player_dbu_nr)
                if player.blank?
                  Rails.logger.info "==== scrape ==== Player #{fl_name} with dbu_nr #{player_dbu_nr}, league cc_id: #{@league.cc_id}, #{@league.name} not in dbu?! Try search by name only"
                  player = Player.find_by_fl_name(fl_name)
                end
                if player.blank?
                  Rails.logger.info "==== scrape ==== Player #{fl_name} with dbu_nr #{player_dbu_nr}, league cc_id: #{@league.cc_id}, #{@league.name} not in dbu?! Looking up or creating..."
                  ...
                  SeasonParticipation.where(sp_args).first ||
                    (sp = SeasonParticipation.new(sp_args)
                     sp.region_id = @region_id
                     sp.global_context = @global_context
                     sp.save)
                else
              ...
              player ||= Player.find_by_fl_name(fl_name)
              if player.blank?
                msg = "==== scrape ==== Fatal ErrorPlayer #{fl_name} with cc_id in region #{@league.organizer.shortname}, league cc_id: #{@league.cc_id}, #{@league.name} #{player_dbu_nr} not in region??!"
  ...
  def parse_parties(league_doc, url)
    ...
    table.css("tr").each do |tr|
      if tr.css("th").count > 0 && tr.css("th")[0][:class] == "bb1"
        ...
        elsif header == ["ST", "TERMIN", "HEIM", "", "GAST"] || header == ["ST", "TERMIN", "HEIM", "", "GAST", "GASTGEBER"]
          if shift == 1
            td_ = tr.css("td")[0]
            if (remark_a = td_.css("a")).present?
              remarks = remark_a[0]["title"].gsub("Memo: ", "")
        ...
        if result_a.present?
          ...
          if td_.present?
            ...
            if location_name.present?
              ...
              location.dbu_nr = location_dbu_nr if /billard-union.net/.match?(url)
              location.source_url ||= url + location_link unless /billard-union.net/.match?(url)
              if location.changed?
              ...
              unless club_location.present?
                (cl = ClubLocation.new(club: club, location: location)
                 cl.region_id = @region_id
                 cl.global_context = @global_context
                 cl.save)
              end
            end
          end
          ...
          if party.blank? || party.party_games.blank? || !@opts[:optimize_api_access]
            ...
            games_per_round = 0
            tables = @game_plan_data[:tables].presence || 0
              ...
              if tr_g.css("th").count.positive?
                if tr_g.css("th").count == 1 && tr_g.css("th").text.strip == "SPIELBERICHT"
                  ...
                  if header_g == %w[Spieltag Termin Heim Erg. Gast Punkte]
                    ...
                    seqno = nil
                    row_index += 1
                    @game_plan_data[:rows][row_index] = {type: "Neue Runde", r_no: r_no}
                ...
                if tr_g.css("td")[0].text.strip.blank? && tr_g.css("td")[1].text.strip.present?
                  if /Bälle:|Punkte:/.match?(tr_g.text)
                    ...
                    @game_plan_data[:rang_mgd] = true
                    dis_name = @game_plan_data[:rows][row_index][:type]
                  ...
                  if /Frame \d+:/.match?(tr_g.text)
                    ...
                    if player_a.blank?
                      player_a = Player.joins(:season_participations).where(season_participations: {
                        club_id: league_team_a.club_id, season_id: @league.season_id
                    ...
                    if player_b.blank?
                      player_b = Player.joins(:season_participations).where(season_participations: {
                        club_id: league_team_b.club_id, season_id: @league.season_id
                  ...
                  row_index += 1
                  @game_plan_data[:rows][row_index] = {type: "Gesamtsumme"}
                else
                  players_a = []
                  players_b = []
                  ...
                  if /shootout/i.match?(discipline_name)
                    @game_plan_data[:victory_to_nil] = [@game_plan_data[:victory_to_nil].to_i, non_shootout_games].max
                    non_shootout_games = 0
                  else
            ...
            if res.present?
              @game_plan_data[:match_points][:win] = [@game_plan_data[:match_points][:win].to_i, res[1]].max
              @game_plan_data[:match_points][:lost] = [@game_plan_data[:match_points][:lost].to_i, res[0]].min
              ...
              if party_games_select.present?
                ...
                if data_result != "0:0"
                  @game_plan_data[:extra_shootout_match_points] = {
                    win: res[1] - res[0],
                    lost: 0
                  }
                  @game_plan_data[:match_points][:draw] = res.min
                end
              end
            end
          end
  ...
  def build_game_plan(disciplines)
    return unless @game_plan_data && @game_plan_data[:rows].present?
    ...
    disciplines.each do |k, v|
      ...
      @game_plan_data[:rows] = @game_plan_data[:rows].map do |row|
        row_ = row
        row_ = row_.merge(v_).compact if row_[:type] == k
        row_.sort.to_h
      end
    end
    @game_plan_data = @game_plan_data.sort.to_h
  end

  def save_game_plan
    footprint = Digest::MD5.hexdigest(@game_plan_data.inspect)
    ...
    gp.assign_attributes(footprint: footprint, data: @game_plan_data)
    ...
    @league.game_plan = gp
    if @league.changed?
      @league.global_context = @global_context
    end
    @league.save!
  end
end
