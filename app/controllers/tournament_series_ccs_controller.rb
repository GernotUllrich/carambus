class TournamentSeriesCcsController < ApplicationController
  before_action :set_tournament_series_cc, only: [:show, :edit, :update, :destroy]
  ...
  def tournament_series_cc_params
    params.require(:tournament_series_cc).permit(:cc_id, :name, :branch_cc_id, :season, :valuation, :series_valuation, :no_tournaments, :point_formula, :min_points, :point_fraction, :price_money, :currency, :club_id, :show_jackpot, :jackpot, :status, :data)
  end
end
