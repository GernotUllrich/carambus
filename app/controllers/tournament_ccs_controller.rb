class TournamentCcsController < ApplicationController
  ...
  def tournament_cc_params
    params.require(:tournament_cc).permit(:cc_id, :context, :name, :shortname, :status, :branch_cc_id, :season, :registration_list_cc_id, :registration_rule, :discipline_id, :championship_type_cc_id, :category_cc_id, :group_cc_id, :tournament_start, :tournament_series_cc_id, :tournament_end, :starting_at, :league_climber_quote, :entry_fee, :max_players, :location_id, :location_text, :description, :poster, :tender, :flowchart, :ranking_list, :successor_list)
  end
end
