...
module International
  ...
  class TournamentsController < ApplicationController
    ...
    def index
      ...
      all_tournaments = all_tournaments.select { |t| t.try(:official_umb?) } if params[:official_umb] == "1"
    ...
    def show
      ...
      # Alle Videos dieses Turniers (direkt mit Turnier verknüpft oder über eines seiner Games)
      @videos = Video.where(
        "(videoable_type = 'Tournament' AND videoable_id = :t_id) OR (videoable_type = 'Game' AND videoable_id IN (:g_ids))",
        t_id: @tournament.id, g_ids: game_ids
      ).recent
