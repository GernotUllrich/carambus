class IonContentsController < ApplicationController
  before_action :set_ion_content, only: [:show, :edit, :update, :destroy]
  ...
  def ion_content_params
    params.require(:ion_content).permit(:page_id, :title, :html, :level, :scraped_at, :deep_scraped_at, :ion_content_id, :data, :hidden, :position)
  end
end
