...
If the build fails, do **not** mark this task done. Fix the underlying issue (in the appropriate file) and re-run.
  ...
  <verify>
    <automated>cd /Users/gullrich/DEV/carambus/carambus_api &amp;&amp; mkdocs build --strict 2>&amp;1 | tee /tmp/36a-mkdocs-strict.log &amp;&amp; WARN=$(grep -c "WARNING" /tmp/36a-mkdocs-strict.log) &amp;&amp; ERR=$(grep -c "ERROR" /tmp/36a-mkdocs-strict.log) &amp;&amp; test "$ERR" -eq 0 &amp;&amp; test "$WARN" -le 191 &amp;&amp; echo "PASS: $WARN warnings (baseline 191), $ERR errors"</automated>
  </verify>
  <acceptance_criteria>
