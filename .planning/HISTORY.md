...
| 260507-c4o | 2026-05-07 | Trap-Context-Bug-Fix in `lib/mcp_server/transport/boot.rb:22` — `Rails.logger.info` ersetzt durch `$stderr.write` im SIGINT/SIGTERM-Handler. Ruby raised `ThreadError: can't be called from trap context` weil Logger einen Mutex akquiriert. Direkter STDERR-Write ist trap-context-safe und behält die Diagnose-Ausgabe beim Strg-C. Pitfall 8 (40-RESEARCH.md §4) jetzt auch im Trap-Body inline dokumentiert. Single-Datei-Fix, kein Test (Signal-Trap-Verhalten nicht unit-testbar ohne Subprocess-Spawn). | `f28b40b5` |
