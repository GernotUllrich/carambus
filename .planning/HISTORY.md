...
| 260506-o93 | 2026-05-06 | **Closed Layer 4 — 36B-06 now 4/4 GREEN.** Added `UI_07_SENTINEL_VALUES = { sets_to_play: [0, 999], sets_to_win: [0] }` constant + 1-line guard in `verify_tournament_start_parameters` to exempt form sentinel values (`["-", 0]` / `["no limit", 999]` from the select_tag options) from range checks. New integration regression test at `test/integration/tournament_verification_sentinels_test.rb` (6 tests, 0.27s) covers positive sentinel-exempt + negative out-of-range + field-scope boundary cases. Result: 36B-06 = 4 runs / 0 failures / 0 skips ✅ — closes the test-driven discovery saga that started with quick-260506-i6h's un-skipping. 36B-05 still 3/3; integration suite 18/18 (12 baseline + 6 new); no other regressions. Production-edge bug fixed: single-set tournaments no longer trip the verification modal. | `0f852f34` + `ab513106` |
| 260506-pha | 2026-05-06 | **Closed Layer 3 — Region picker submenu replaces hardcoded Region[1] link.** Plan iteration 1 PASSED with the wrong fix shape (presence-guard); user interrupted execution to clarify: "Der Migrationslink macht nur Sinn, mit 1 Region ID. Einen Default dort anzunehmen, geht nicht. Es soll ja auf eine ganz bestimmte Region migriert werden. Also muss dort eine Frage kommen, welche Region gemeint ist." Plan iteration 2 revised to nested-submenu picker: replaced `link_to "Migration", migration_cc_region_path(Region[1])` (would crash with `UrlGenerationError` when Region[1] is nil) with a nested submenu (button + `<ul>`) iterating `Region.order(:shortname, :name)`, each Region getting its own `migration_cc_region_path(region)` link. Matches existing 8-instance Sidebar submenu pattern at `_left_nav.html.erb:21-37`; uses existing Stimulus `sidebar_controller#toggle` (depth-agnostic via `nextElementSibling`, no JS edit). New integration test `test/integration/left_nav_system_admin_test.rb` (3 tests / 15 assertions / 0 failures) asserts (a) sidebar renders 200 under system_admin, (b) NBV `shortname` link to `/regions/50000001/migration_cc` is emitted, (c) no `migration_cc_region_path(nil)` artifact remains. 36B-06 still 4/4 (no regression). Integration suite now 21/21. Production-edge fix: system_admin users on local servers without Region[1] no longer see a 500; admins now also have a usable per-Region picker instead of a single hardcoded link. | `4568b2a0` |
| 260507-24p | 2026-05-07 | **Phase-39 Gap-01 fix — `Discipline#lookup_dtp_with_class_walk` now filters `tournament.seedings.count` by `Seeding::MIN_ID`.** UAT discovery: verification modal never fired on real tournaments because `seedings.count` included globally synced seedings (id < `Seeding::MIN_ID = 50_000_000`), inflating the count above the actual local-tournament player count → no DTP row matched → `parameter_ranges` returned `{}`. Phase-39 tests missed it because all fixture seedings already had IDs ≥ MIN_ID. Fix: extracted `effective_player_count(tournament)` private helper using the smart-fallback pattern from `tournament_cc.rb:286` (local count if any local seedings exist, else fall back to global count) — works on both local servers and the central API server. Added 3 globally-id'd fixture seedings attached to existing `local_fpk_class1` + 2 regression tests (mixed-seedings local-path + only-global API-server-path). 24/24 tests GREEN in `discipline_test.rb` (22 pre-existing + 2 new). Phase 39 UAT Tests 2-7 now unblocked. Orthogonal `tournament.player_class=nil` data-migration concern (Bestands-Turniere wie *„NDM Cadre 35/2 Klasse 1"*) deliberately out-of-scope — to be addressed separately. | `fa5c9529` + `6495a293` + `d380da6b` |
