...
Last activity: 2026-05-06 - Completed quick task 260506-k3t (commits `2fcce9d1`, `d6335bff`, `6f8a6f52`, `e362f8a9`): closed both DEFERRED-BLOCKERs from 260506-i6h. Bug 1: payload string keys + JSON round-trip regression test (2/2 green). Bug 2: `:local` fixture state → `tournament_mode_defined` (AASM-correct). **Rule 1 latent-bug:** AASM event `:start_tournament!` has literal bang in symbol — auto-generated method does in-memory transition only; added explicit `@tournament.save`. Production code now correct end-to-end (commit `0ac7305a` validated). **Verifier: human_needed** — system tests 3+4 still fail at assertion layer due to test-thread/Puma connection isolation (orthogonal test-infra gap). Recommendation: SHIP the 9-commit linear stack. Today's earlier work: 260506-hka (PRG refactor) + 260506-i6h (fixture FK rot + 36B-05 tightening). **Pre-push: 9 commits ahead of origin/master in bcw; other checkouts clean.**
...
**System-test connection isolation (test-infra; not blocking push):** 36B-06 tests 3+4 fail at `assert_includes STARTED_STATES, @tournament.reload.state` even though the controller's `UPDATE` is visible in `log/test.log`. Root cause: `application_system_test_case.rb` has no shared-connection setup, so the test thread reads a stale snapshot from its own Postgres connection while the Puma server thread's UPDATE flushes on its connection. Fix options (each affects ALL system tests asserting controller-side state changes): (a) install `database_cleaner` with `:truncation` strategy + drop `use_transactional_tests`; (b) explicitly share the connection via the standard Rails 6.1+ pattern; (c) rewrite assertions to check DOM/URL state instead of DB state. Recommend option (b) — minimal blast radius. File a dedicated quick task before further system-test-driven work in this area.
