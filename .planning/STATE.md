...
Last activity: 2026-05-06 - **Quick task 260506-lii HALTED with zero net change** while attempting to address the system-test connection-isolation issue. Discovered a 3-layer cascade where each fix exposes the next: Layer 1 (`users(:admin)` bounced by `ensure_tournament_director`), Layer 2 (Postgres test-thread/Puma-thread connection isolation — original hypothesis, now likely moot), Layer 3 (`_left_nav.html.erb:156` crashes with `UrlGenerationError` when `Region[1]` is nil under `:system_admin`). Layers 1+3 are coupled: any fixture user satisfying `ensure_tournament_director` also activates the broken admin nav. Two plan iterations used, working tree clean at `8914f567`. Earlier today (already pushed): 260506-hka (PRG refactor) + 260506-i6h (fixture FK rot + 36B-05) + 260506-k3t (closed DEFERRED-BLOCKERs + AASM persistence). bcw at origin/master HEAD; other checkouts 10 commits behind (intentional, staying in Debugging Mode).
...
**Get 36B-06 system tests 3+4 to pass — three-layer cascade discovered by halted quick-260506-lii:** What looked like a single test-infra issue is actually three coupled problems, only the first of which can be fixed in isolation:

- **Layer 3 (root — must fix first):** `app/views/application/_left_nav.html.erb:156` calls `migration_cc_region_path(Region[1])`. In test fixtures `Region[1]` is nil (`:nbv` is at id 50_000_001). Under any system_admin user this raises `UrlGenerationError` → 500 → start_tournament form never renders. Fix options: (a) harden the view (`migration_cc_region_path(Region[1] || Region.first)` or wrap link in `if Region[1].present?`); (b) add a `Region` fixture at id 1; (c) gate the link on `Region[1].present?` AND user role. Recommend (a) — minimal-blast hardening of a real production-edge bug.

- **Layer 1 (depends on Layer 3 fix):** Test setup uses `users(:admin)` which lacks `role:`, so `TournamentMonitorsController#ensure_tournament_director` (lines 201-206) bounces the success-path redirect from `/tournament_monitors/:id` to `/`. After Layer 3 is hardened, swap to `users(:system_admin)` (already exists at `test/fixtures/users.yml:106-115`) so the redirect chain settles at the success URL.

- **Layer 2 (likely moot after Layer 1 + 3 fixed, but verify):** Original hypothesis — test-thread / Puma-server-thread Postgres connection isolation under `use_transactional_tests = true` causes test thread to read stale `@tournament.reload.state`. Once Layer 1 unblocks the URL endpoint, rewrite the 2 failing assertions in `tournament_parameter_verification_test.rb:85, 99` to assert `assert_current_path %r{\A/tournament_monitors/\d+\z}` instead of `@tournament.reload.state`. This routes around the connection-isolation question entirely. The original hypothesis becomes both untestable and unimportant.

Project hard constraints (preserved across all attempts): **do NOT** disable `use_transactional_tests` globally (`test/TEST_DATABASE_SETUP.md:94` documents reliance), **do NOT** introduce `database_cleaner`, **do NOT** modify `test_helper.rb` / `application_system_test_case.rb` / `Gemfile` / `users.yml`, **do NOT** add a new test base class or mixin (extend-before-build SKILL).

Recommended: file a new quick task scoped to "fix Layer 3 (`_left_nav.html.erb` Region[1] nil-handling) first; then in same task or follow-up, swap fixture user + rewrite assertions". The Layer 3 fix is itself a real production hardening worth shipping regardless of the test work.
