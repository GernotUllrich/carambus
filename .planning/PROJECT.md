...
---
*Last updated: 2026-04-29 after Phase 38.5 (BK-* param hierarchy & multiset config) — extracted BK-* score engine logic from `free_game_form` string-equality predicates to data-key-driven predicates backed by new `BkParamResolver` service. Resolver walks the 7-level hierarchy (Discipline → Tournament → ... → TableMonitor) with sparse-override semantics and per-set `effective_discipline` for BK-2kombi multisets. Wired into `GameSetup#start_game` (initial bake) and `ResultRecorder#switch_to_next_set` (per-set rebake via `Bk2::AdvanceMatchState.rebake_at_set_open!`). Two latent bugs fixed: D-11 BK-2kombi DZ opponent-credit, D-12 BK-2 negative-stays-signed. 12/12 must-haves verified, 138 directly-tested runs PASS. 13 errors + 2 failures in `bk2_scoreboard_test.rb` documented as pre-existing Phase 38.4 tech debt (`Bk2::CommitInning` namespace consolidation) in `deferred-items.md`. 5 Warning-level review findings documented in `38.5-REVIEW.md` for follow-up.*
