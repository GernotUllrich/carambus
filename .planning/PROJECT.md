...
- [x] **G-06 / DATA-01**: `Discipline#parameter_ranges(tournament:)` is DTP-backed (queries `discipline_tournament_plans` with class-walk fallback + lenient-OR Range derivation); returns `{}` for handicap, no-plan, blank-class, non-DTP, walk-miss, and zero-canonical (Cup-series) cases — closed in Phase 39 (2026-05-06)
...
---
*Last updated: 2026-05-16 after Phase 41 (Devise-Auth-Cluster Überarbeitung — Registrierung, Forgot-Password, Change-Password, Email-Change Flows konsolidiert) — alle vier D-41-D-Flows produktionsreif gemacht und mit 4-Layer-Test-Pyramide (Model+Controller+Mailer/Job+System-E2E) abgesichert: (1) Test-Infra + letter_opener_web-Aktivierung im Dev + Charakterisierungstests für IST-Zustand aller 4 Flows (`test/support/mail_helpers.rb`, `devise_test_helpers.rb`, Layer-3 Mailer-Tests, Layer-2 Passwords/Confirmations-Controller-Tests). (2) Registrierungs-Flow gehärtet: `invisible_captcha` server-seitig erzwungen (vorher nur View-Render — latentes Spam-Loch), `terms_of_service`-Checkbox via `:sign_up`-Permit-Sanitizer akzeptiert (vorher nie validiert — Acceptance-Validation griff nicht) + i18n DE/EN. (3) Forgot-Password JTI-Hard-Revoke via `User#after_update :rotate_jti_on_password_change!, if: :saved_change_to_encrypted_password?` — Passwort-Reset rotiert JTI, alle JWT-Sessions invalidiert (kompromittierter Account, der per Reset gesichert wird, kann mit gestohlenem JWT nicht weiter genutzt werden). Cross-Repo-sicher: Routine-Updates (first_name, email) triggern JTI nicht — bcw-MCP-JWTs überleben. (4) D-41-C semantisch durchgesetzt: `config.sign_in_after_change_password = false` — User muss nach PW-Change neu einloggen (Re-Login-Pflicht zwingt Browser-Session-Hard-Revoke). (5) SMTP-Härtung komplett gemäß D-41-B: `config/initializers/smtp_guard.rb` (Production-Fail-Fast bei fehlendem `SMTP_USERNAME`/`SMTP_PASSWORD` — verifiziert: `RuntimeError: FATAL: SMTP-ENV nicht gesetzt` an `smtp_guard.rb:17`), `config/initializers/mail_observer.rb` (MailDeliveryObserver tagged 'MAILER' für Success+Failure-Logging), `app/jobs/devise_mail_job.rb` (retry_on `Net::SMTPAuthenticationError` 3 attempts + `Net::SMTPServerBusy` 5 attempts mit polynomial backoff + discard_on `Net::SMTPFatalError` mit tagged Bounce-Logging), `User#send_devise_notification`-Override queued via `DeviseMailJob.perform_later` statt synchronem `deliver_now` (User-Request wird nicht mehr durch SMTP blockiert). (6) Sender-Angleichung gegen SPF/DKIM-Mismatch (T-41-04-03): `ApplicationMailer.default from: proc { Devise.mailer_sender }` — `Proc` statt `Lambda` notwendig wegen ActionMailer-Internals (Block wird mit Mailer-Instance-Arg gerufen). (7) E2E System-Tests via Capybara/Selenium für alle 4 D-41-D-Flows in `test/system/devise_flows_test.rb` — 5-Seed-Stabilität verifiziert, async-race-safe via `wait_for_mail`+`wait_until` Polling-Helper, invisible_captcha-Threshold per `InvisibleCaptcha.timestamp_threshold = 0` overridden (Capybara-Server-Thread macht `travel` nicht zuverlässig sichtbar). End-to-End: 58/58 Phase-41-Tests grün (54 Layer-1+2+3 mit 171 Assertions + 4 Layer-4 System-Tests mit 24 Assertions). User-Walkthrough am 2026-05-16 im carambus_nbv-Scenario (Port 3301) verifiziert alle 8+1 Schritte (Sign-up → Confirmation → Forgot-PW → Change-PW → SMTP-Guard-Production-Fail-Fast). Production-SMTP-Roundtrip-Verifikation (Plan 41-05 Task 2) explizit als deferred Approval geführt — manueller Smoke-Test mit echter Gmail-Inbox post-deploy erforderlich (CONTEXT.md Anti-Pattern). 3 deferred Items aus Walkthrough dokumentiert: DI-41-04-01 (Plan-Walkthrough-URLs zeigten Standard-Devise-Pfade statt Custom-Routes `/password/new`, `/login`), DI-41-04-02 (Mail-Default-URL `lvh.me` löst bei DNS-Resolver-Problemen nicht auf — Workaround: `localhost` in URL, oder `127.0.0.1 lvh.me` in `/etc/hosts`), DI-41-04-03 (`development-carambus.rb` letter_opener_web-Eintrag redundant mit `development.rb:113`). Verifikation: 18/18 must-haves PASS via gsd-verifier (goal-backward).

Previously: Phase 40 (MCP Server für ClubCloud-Schnittstelle) — built an in-Carambus-eingebetteten Ruby MCP-Server (`bin/mcp-server` + `lib/mcp_server/`, Stdio-Transport, Anthropic Official `mcp` Gem ~> 0.15.0) der vier Wissens-Schichten an Claude Desktop / Claude Code exponiert: (1) 5 DE-Workflow-Resources (`cc://workflow/scenarios/{teilnehmerliste-finalisieren,player-anlegen,endrangliste-eintragen}` + `cc://workflow/{roles,glossary}`), (2) 15 curated `cc://api/{action}` Resources (D-04 Allowlist über `RegionCc::ClubCloudClient::PATH_MAP`, kein Auto-Mapping, mit Drift-Guard), (3) 10 EN-Read-Tools (`cc_lookup_{region,league,tournament,teilnehmerliste,team,club,spielbericht,category,serie}` + `cc_search_player`) — 4 DB-first via `Region`/`RegionCc`/`LeagueCc`/`TournamentCc` + Force-Refresh-Fallback, 6 live-only via `RegionCc::ClubCloudClient.get` —, (4) Write-Architektur (Allowlist + JSON-Schema + `armed`-Flag + D-11 trust-CC-and-parse-error) plus Proof-Implementierung `cc_finalize_teilnehmerliste` (PATH_MAP `releaseMeldeliste`) gegen Mock-Backend mit Login-Redirect-Reauth-Retry. Credentials kommen per MCP-Client-Installation (`mcp.json`: `CC_USERNAME`/`CC_PASSWORD`/`CC_FED_ID`/`CARAMBUS_MCP_MOCK`); kein Secret im Repo. `cc_session.rb` delegiert Login an `Setting.login_to_cc` (kein Hand-rolled Net::HTTP), 30-Min-TTL, Production+Mock-Mode → `RuntimeError`. Server.build verwendet einen ZENTRALEN `resources_read_handler`-Dispatcher (Plans 02+03 registrieren keinen eigenen Handler); 11 Tools (10 read + 1 write) und 20 Resources (5 workflow + 15 api) sind via Konstanten-Lookup automatisch registriert. End-to-End: 65 Test-Runs / 220 Assertions / 0 failures (8 CcSession + 6 Server-Smoke + 11 Resource + 10 Tool + 11 Lookup-Smoke + 6 finalize + 6 stdio-E2E + 7 ApiSurface-Drift). Setup-Doku liefert `.mcp.json.example`, Manager-Setup (Sportwart, Claude Desktop) und Developer-Setup (Carambus-Dev, Claude Code) jeweils auf Deutsch. Capistrano-Task `lib/capistrano/tasks/mcp_server.rake` setzt `bin/mcp-server` nach `bundle:install` automatisch auf 0755 (RESEARCH Open Question §5 RESOLVED). `MCP::Tool::Response` exponiert `#error?` (Predicate, nicht `#error`) — Plan-01-SDK-API-Probe sperrt das ab. Code-Review (advisory): 0 critical / 5 warning / 7 info — die 5 Warnings betreffen DB-first-Lookup-Pfade (WR-01 falsche `tournament_cc`-Assoziation, WR-02/03 League-Query ohne `fed_id`/`branch_id`), die durch Mock-only-Tests verdeckt sind und erst bei realen CC-Usern auftreten; nicht-blockierend, Folge-Phase 40.1 für Restliche-Write-Tools wird die Lookup-Pfade ebenfalls anfassen. Verifikation: 9/9 Roadmap-Success-Criteria erfüllt.

Previously: Phase 39 (DTP-Backed Parameter Ranges) — closed DATA-01/G-06 by replacing the hardcoded `Discipline::DISCIPLINE_PARAMETER_RANGES` constant + 2 helper constants (`UI_07_SHARED_RANGES`, `UI_07_DISCIPLINE_SPECIFIC_RANGES`) with a DTP-backed `Discipline#parameter_ranges(tournament:)` keyword-arg method that queries `discipline_tournament_plans` by (discipline, plan, players=seedings.count, player_class) with class-walk fallback (`PLAYER_CLASS_ORDER` worst→best, walk to stricter on miss) and lenient-OR Range derivation (`(canonical * 0.75).floor..canonical`, REDUCED_FACTOR=0.75 corrected from ROADMAP's 0.8 per CONTEXT D-07). Returns `{}` for 6 early-return branches: `handicap_tournier`, `tournament_plan_id` nil, blank `player_class`, no DTP hit, walk-miss, and zero-canonical (Petit/Grand Prix + Nordcup Cup-series semantic where score-target lives per-Discipline-Tournament). Plan 02 narrowed `UI_07_FIELDS` from 7→2 (`balls_goal`, `innings_goal` — operator-input fields removed), deleted the 13-line `UI_07_SENTINEL_VALUES` constant + 3-line sentinel guard + 107-line `tournament_verification_sentinels_test.rb` (7 tests of dead code per RQ-04), migrated the single production caller in `tournaments_controller.rb#verify_tournament_start_parameters` to keyword-arg form, and added 3 new no-fire system tests (BK-2kombi non-DTP, handicap_tournier=true, tournament_plan=nil). Verifier: 12/12 must-haves PASS; full sweep `bin/rails test test/controllers test/models test/integration` → 757 runs / 2056 assertions / 0 failures / 0 errors / 8 pre-existing skips. Code review: 0 critical / 3 advisory warnings (WR-01 PLAYER_CLASS_ORDER mixes numeric+roman classes — latent, no live mixing; WR-02 `seedings.count` not cache-aware; WR-03 asymmetric DTP rows uncovered by tests). Cross-repo regex sweep CLEAN: `DISCIPLINE_PARAMETER_RANGES|UI_07_SHARED_RANGES|UI_07_DISCIPLINE_SPECIFIC_RANGES|UI_07_SENTINEL_VALUES` returns 0 matches across `app/` + `test/`. The verification modal will no longer false-fire on Pool/Snooker/BK-/Biathlon/Kegel/handicap tournaments — non-DTP and handicap branches return `{}` (= no check). v7.1 milestone progress: 70/70 plans complete (100%); only Backlog 999.1/999.2 remain.

Previously: Phase 38.9 (BK-2 / BK-2kombi-SP end-of-set Anstoss-at-goal-in-inning-2 fix) — closed a latent defect introduced together with the Erste-Aufnahme-Gate at commit `79328663` (2026-04-28): when the Anstoss-Spieler reached `balls_goal` in his 2nd+ inning, `TableMonitor#end_of_set?` returned `false` because none of the three existing branches matched (Branch 1 excluded BK-2; Plan 38.7-02 D-02 required Nachstoss already played; legacy karambol parity required innings parity OR `!allow_follow_up`). The set stayed open, `ScoreEngine#terminate_inning_data` swapped `active_player`, Player B played an unauthorized inning, and the legacy parity gate eventually closed the set without ever showing the red "Nachstoß" banner. Fix: 4th sub-branch added inline inside the existing Plan 38.7-02 D-02 outer-`if` block (`app/models/table_monitor.rb:1529-1552`) — `if anstoss_at_goal && anstoss_innings >= 2; return true; end` — the close-side mirror of `follow_up?`'s Erste-Aufnahme-Gate at lines 1205-1210. Reuses already-computed locals from D-02 (zero recomputation). 4/4 SC verified (RED 78b0dfbc → regression 4d275e69 → GREEN d231f9d7 → docs a58f2772). 21/21 unit + 4/4 Phase 38.7 tiebreak + 4/4 Phase 38.8 operator-gate seals all GREEN. Code review: 0 critical / 0 warning / 3 info. NOT-to-modify list honored (`follow_up?`, `score_engine.rb#terminate_inning_data`, `_player_score_panel.html.erb` all unchanged). 3/3 human UAT scenarios pass; one non-blocking UI observation captured at backlog 999.1 (Endergebnis-erfasst Winner label + per-set result on score panel). 19 pre-existing failures in `test/system/bk2_scoreboard_test.rb` (Phase 38.5/38.6 cleanup debt) verified at parent commit `cfee5962` — NOT regressions from 38.9; deferred to a future cleanup quick-task. SKILL extend-before-build applied (validated 2026-04-29 with -1463 LOC rollback on this exact surface): additive guard, no parallel state machine.

Previously: Phase 38.8 (Endergebnis-erfasst state restore — operator-gate the post-match flow across all disciplines) — restored the AASM `:final_match_score` operator-gate by deleting the auto-rematch blocks at `result_recorder.rb:462-476/485-497` (Branch C + final_set_score, regression introduced commit c3dedb69 2026-03-24), adding `:start_rematch` AASM event to TableMonitor with `revert_players` + `do_play` after-callbacks, deferring the tournament round-progression cascade out of `report_result` into a new public `advance_round_after_match_close` method wired via `:close_match` after-callback, and wiring two new reflex actions (`start_rematch` / `close_match`) plus a `<%- elsif final_match_score? %>` branch in `_scoreboard.html.erb` that renders a "Nächstes Spiel" / "Next Game" button. 5/5 SC verified, 89-test regression sweep GREEN, Phase 38.7 tiebreak suite preserved (4/4 system tests). Code review found 2 critical findings: CR-01 (PartyMonitor polymorphism crash in `advance_tournament_round_if_present` — fixed with `is_a?(TournamentMonitor)` guard at `f2e438d2`) and CR-02 (`:close_match` after-callback recursion via `TournamentMonitorState#finalize_round` — flagged via static read but **reclassified as false positive** after user manually validated BK-2 round_play; sentinel reverted at `0dbe45c4`+`61e9e8c8`). 4 warnings auto-fixed (`69ea5726`/`fac941dd`/`2db0bce6`/`aea44369`). One human-verification item remains (visual UX across disciplines) — surfaces in `/gsd-progress` until run.

Previously: Phase 38.7 (Tiebreak bei Unentschieden — per-game flag with modal) — 6/6 plans + 5 gap-closure plans (38.7-09 through 38.7-13). Plan 13 closed Gap-05 (operator-gate post-match modal infinite-loop blocker) by relocating `data-reflex="submit->..."` from `<button>` to `<form>` element in `_game_protocol_modal.html.erb`.

Previously: Phase 38.6 (Discipline Master-Data Cleanup — BK-* duplicates on carambus_api) — built `script/merge_bk_disciplines.rb` (482 lines, PaperTrail-aware) that consolidates 6 duplicate BK-* `Discipline` rows (losers 57/59/60/61/62/95) into 5 canonical winners (107/108/109/110/111), preserves all 17 ActiveRecord reflections via `update!`-based FK reassignment, renames winner 107 "BK2-Kombi" → "BK-2kombi" (D-12), and propagates loser names + existing aliases into each winner's `Discipline#synonyms` text column before destroy so scrapers (UMB/Cuesco/SoopLive/Ko-Zoom) retain legacy lookup paths. Plan 02 lexically aligned `BK2_DISCIPLINE_MAP`, `BK_FAMILY_BALLZIEL_FALLBACK`, fixture and test references to the canonical name. Plan 03 dry-ran the script against `carambus_api_development` with transaction rollback (Discipline.count = 79 unchanged, 0 CC conflicts, 0 PlayerRanking recompute conflicts, all 6 merges succeed). Plan 04 (gap closure on user directive) added the synonyms preservation block + per-winner synonyms emit in post-flight protocol. 8/8 must-haves verified by goal-backward gsd-verifier. Code review: 0 blockers, 4 advisory warnings (WR-01 stale `BK_NAME_TO_FORM` key in `app/services/table_monitor/game_setup.rb`, WR-02 dead rescue, WR-03 dry-run exit-bypass, WR-04 seed-replay carve-out tests). The actual cleanup run (manual `RAILS_ENV=development bin/rails runner script/merge_bk_disciplines.rb` against `carambus_api_development` — the canonical DB on this machine that local servers sync from via `Version#update_from_carambus_api`) is explicitly out-of-scope for this phase; recommended follow-up before that run: WR-01.

Previously: Phase 38.5 (BK-* param hierarchy & multiset config) — extracted BK-* score engine logic from `free_game_form` string-equality predicates to data-key-driven predicates backed by new `BkParamResolver` service. Resolver walks the 7-level hierarchy (Discipline → Tournament → ... → TableMonitor) with sparse-override semantics and per-set `effective_discipline` for BK-2kombi multisets. 12/12 must-haves verified, 138 directly-tested runs PASS.*
