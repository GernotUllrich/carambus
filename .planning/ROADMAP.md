...
Plans:
- [x] `38.1-01-engine-negative-score-gate-PLAN.md` — Bypass the three `score_engine.rb` negative-score gates (:84 guard, :135 clamp, :690-692 protocol rejection) for `data["free_game_form"] == "bk2_kombi"` via a new `allow_negative_scores?` helper. Includes characterization tests that prove karambol still clamps/rejects. Wave 1.
- [x] `38.1-02-dispatch-and-discipline-data-PLAN.md` — `GameSetup` derivation from `discipline.data["free_game_form"]` with name-match fallback and warning log; `OptionsPresenter` passthrough verification; `ResultRecorder` BK2 dispatch branch to `Bk2Kombi::AdvanceMatchState`; D-10 `discipline.data` write on id 107 (primary carambus_api path + BCW unprotected fallback with reconciliation runbook). Wave 1.
- [x] `38.1-03-bk2-kombi-scoring-services-PLAN.md` — `Bk2Kombi::ScoreShot` (pure evaluator, DEFAULT_RULES frozen constant, exact D-15/D-16 literals) + `Bk2Kombi::AdvanceMatchState` (set/match close, idempotency via `shot_sequence_number`). TDD with 22 unit + 11 state-mutation + integration tests. Wave 2.
- [x] `38.1-04-scoreboard-partial-and-input-PLAN.md` — Dedicated `_scoreboard_bk2_kombi.html.erb` partial + `_show_bk2_kombi` wrapper, shot-input form, `bk2_kombi_submit_shot` reflex action with scoped CableReady broadcast, Stimulus controller with 500ms submit debounce, DE+EN i18n keys under `table_monitor.bk2_kombi.*`. Wave 3.
- [x] `38.1-05-dry-run-uat-and-fallback-drill-PLAN.md` — Pre-dry-run gate (critical suite + lint + brakeman + production data sanity), physical BCW club-table dry run with explicit GO/NO-GO verdict (D-02 UAT gate), karambol-fallback drill runbook + rehearsal (D-03/D-07), phase closure updates in STATE.md + ROADMAP.md. Wave 4, autonomous:false. **Completed 2026-05-05** (retroactive — Tasks 2/3 covered by real Grand Prix usage 2026-05-02 + club-day post-tournament test; no formal pre-tournament dry-run; carry-forward TODOs A/B/C in STATE.md).
...
**Plans**: 12/13 plans complete + 1 reverted (5 gap-closure plans 38.7-09…13 landed 2026-04-30 → 2026-05-01)
...
- [x] `38.7-09-PLAN.md` — Gap-01: `config/carambus.yml` `quick_game_presets` per-button `tiebreak_on_draw: true` on BK-2 + BK-2kombi; threaded through form → controller → GameSetup preset-override block (deep_merge_data!). 3 RED→GREEN tests. Wave 1. (commits `0f40eb2d` / `15838186` / `e5c3b708`)
- [x] `38.7-10-PLAN.md` — Gap-02: free_game detail-form checkbox (`scoreboard_free_game_karambol_new.html.erb`) + i18n (DE/EN). Reuses Plan 09 controller wiring. 2 controller tests. Wave 2 (depends on 38.7-09). (commits `4cb60d73` / `ece7fc13` / `caa97e83`)
- [x] `38.7-11-PLAN.md` — Gap-03: BK-2kombi BK-2-phase auto-detect (`bk2_kombi_tiebreak_auto_detect!` private helper inside `tiebreak_pick_pending?`) — fires when phase=serienspiel + 1+1 innings + tied at goal; overrides any pre-baked false. 3 RED→GREEN tests. Wave 1. (commits `dd97be5c` / `f3915960` / `8773f815`)
- [x] `38.7-12-PLAN.md` — Gap-04: TournamentMonitor startup-form checkbox + controller persist to Tournament.data['tiebreak_on_draw']. Default reads TournamentPlan.executor_params['g1'] Level 2. 3 RED→GREEN tests. Wave 3 (depends on 38.7-10, i18n file ownership). (commits `44d9c71f` / `321ff8bf` / `7cfe16b9`)
- [x] `38.7-13-tiebreak-modal-form-reflex-wiring-PLAN.md` — Gap-05: tiebreak modal submit-loop blocker. Move `data-reflex="submit->GameProtocolReflex#confirm_result"` + `data-id` from `<button>` onto `<form>` in `_game_protocol_modal.html.erb` (browsers fire submit events on forms, not buttons); add `action="javascript:void(0)"` defensive no-op fallback. 4 RED→GREEN integration tests (render-based wiring + GET-fallback no-op). Wave 1 (standalone). (commits `3e209190` / `e6baa7d2` / `c3cbf1ef`)
...
| 40. MCP Server für ClubCloud-Schnittstelle | v7.1 | 2/6 | In Progress|  |
...
### Phase 999.1: Endergebnis-erfasst Winner label + per-set result on score panel (BACKLOG)
