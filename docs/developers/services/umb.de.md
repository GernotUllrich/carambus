...
| `Umb::DisciplineDetector` | `app/services/umb/discipline_detector.rb` | Ordnet Turniernamen via Regex und DB-ILIKE-Lookup `Discipline`-Records zu (`detect`); `detect_with_title_fallback` ergänzt einen titelbasierten `Discipline.classify_from_title`-Fallback (genutzt von Future-/Archive-Scraper) |
...
- [UMB Scraping — Architektur](../umb-scraping-implementation.de.md) — Architektur, Datenfluss, Service-Interaktionen
- [UMB Scraping — Methoden-Referenz](../umb-scraping-methods.de.md) — Methodenverzeichnis, Parameterdokumentation
