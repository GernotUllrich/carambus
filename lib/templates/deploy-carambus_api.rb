...
set :application, "carambus2_api"
set :repo_url, "git@github.com:GernotUllrich/carambus2.git"
...
set :deploy_to, "/var/www/carambus2_api"
...
append :linked_files, "config/database.yml", "config/master.key", "config/scoreboard_url", "config/credentials/production.key", "config/environments/production.rb", "config/credentials/production.yml.enc", "config/puma.rb"
...
namespace :puma do
  ...
  task :restart do
    on roles(:app) do
      execute "sudo /var/www/carambus2_api/current/bin/manage-puma.sh"
    end
  end
end
