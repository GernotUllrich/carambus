...
namespace :mode do
  ...
  task local: :environment do
    ...
    carambus_config = {
      ...
      development: {
        carambus_api_url: nil,  # Empty for local mode
        location_id: 1,
        application_name: "Carambus",
        support_email: "gernot.ullrich@gmx.de",
        business_name: "Ullrich IT Consulting",
    ...
    File.write(Rails.root.join('config', 'database.yml'), database_config.to_yaml)
    ...
    puts "Current mode: LOCAL (carambus_api_url is empty, local database)"
  end
  ...
  task api: :environment do
    ...
    puts "Current mode: API (carambus_api_url is set, API database)"
  end
