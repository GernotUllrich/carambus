...
namespace :cleanup do
  ...
  task remove_non_region_records: :environment do
    ...
    deletion_order = [
      { model: SeasonParticipation, keep_ids: season_participation_ids },
      { model: GameParticipation, keep_ids: GameParticipation.where(game_id: game_ids).pluck(:id) },
      { model: Game, keep_ids: game_ids },
      ...
      { model: Table, keep_ids: table_ids },
      { model: Location, keep_ids: location_ids },
      { model: Club, keep_ids: club_ids },
      { model: Region, keep_ids: region_ids }
    ]

    # Delete records in order
    deletion_order.each do |item|
      model = item[:model]
      keep_ids = item[:keep_ids]
