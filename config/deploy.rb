  ...
  task :restart do
    on roles(:app), in: :sequence, wait: 5 do
      ...
      if fetch(:stage).to_s == 'production'
        execute "cd #{current_path} && BEANSTALK_URL=beanstalk://localhost:11300 RAILS_ENV=production ~www-data/.rbenv/shims/bundle exec #{current_path}/script/worker.rb stop"
        execute "cd #{current_path} && BEANSTALK_URL=beanstalk://web2.localhost:11300 RAILS_ENV=production ~www-data/.rbenv/shims/bundle exec #{current_path}/script/worker.rb start"
      end
    end
