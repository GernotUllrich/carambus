# Puma Configuration for carambus_api
# Generated by Carambus Mode System

# Puma can serve each request in a thread from an internal thread pool.
# The `threads` method setting takes two numbers: a minimum and maximum.
# Any libraries that use thread pools should be configured to match
# the maximum value specified for Puma. Default is set to 5 threads for minimum
# and maximum; this matches the default thread size of Active Record.
#
threads_count = ENV.fetch("RAILS_MAX_THREADS") { 5 }
threads threads_count, threads_count

# Specifies the `port` that Puma will listen on to determine requests.
# Default is 3000.
#
# port        ENV.fetch("PORT") { 3000 }

# Specifies the `environment` that Puma will run in.
#
environment ENV.fetch("RAILS_ENV") { "development" }

# Specifies the number of `workers` to boot in clustered mode.
# Workers are forked webserver processes. If using threads and workers together
# the concurrency of the application would be max `threads` * `workers`.
# Workers do not work on JRuby or Windows (both of which do not support
# processes).
#
workers ENV.fetch("WEB_CONCURRENCY") { 2 }

# Use the `preload_app!` method when specifying a `workers` number.
# This directive tells Puma to first boot the application and then code
# for other workers. This takes advantage of Copy On Write process
# behavior so workers use less memory.
#
preload_app!

# If you are preloading your application and using Active Record, it's
# recommended that you close any connections to the database before workers
# are forked to prevent connection leakage.
#
before_fork do
  ActiveRecord::Base.connection_pool.disconnect! if defined?(ActiveRecord)
end

# The code in the `on_worker_boot` will be called if you are using
# clustered mode by specifying a number of `workers`. After each worker
# process is booted, this block will be called. If you are using the `preload_app!`
# option, you will want to use this block to reconnect to any connections
# that may have been created at application boot, as Ruby shared sockets
# between the processes.
#
on_worker_boot do
  ActiveRecord::Base.establish_connection if defined?(ActiveRecord)
end

# Allow puma to be restarted by `rails restart` command.
plugin :tmp_restart

# Unix Socket Configuration
# Use Unix socket for communication with NGINX
app_dir = File.expand_path("../..", __FILE__)
shared_dir = "#{app_dir}/shared"

# Ensure the shared directory exists
Dir.mkdir(shared_dir) unless Dir.exist?(shared_dir)

# Create socket directory
socket_dir = "#{shared_dir}/sockets"
Dir.mkdir(socket_dir) unless Dir.exist?(socket_dir)

# Create pids directory
pids_dir = "#{shared_dir}/pids"
Dir.mkdir(pids_dir) unless Dir.exist?(pids_dir)

# Create log directory
log_dir = "#{shared_dir}/log"
Dir.mkdir(log_dir) unless Dir.exist?(log_dir)

# Bind to Unix socket
bind "unix://#{socket_dir}/puma-#{ENV.fetch('RAILS_ENV') { 'development' }}.sock"

# Set socket permissions
pidfile "#{pids_dir}/puma-#{ENV.fetch('RAILS_ENV') { 'development' }}.pid"
state_path "#{pids_dir}/puma-#{ENV.fetch('RAILS_ENV') { 'development' }}.state"

# Logging
stdout_redirect "#{log_dir}/puma.stdout.log", "#{log_dir}/puma.stderr.log", true

# Set socket permissions after binding
# Note: after_bind is not available in older Puma versions
# Socket permissions will be set by the system

# Ensure socket has correct permissions for NGINX
# Set permissions immediately after binding
socket_file = "#{socket_dir}/puma-#{ENV.fetch('RAILS_ENV') { 'development' }}.sock"
if File.exist?(socket_file)
  File.chmod(0666, socket_file)
  puts "Socket permissions set to 666: #{socket_file}"
end
