class ConvertPlayerDataFromYamlToJson < ActiveRecord::Migration[7.2]
  # Definiere eine temporäre Klasse, die direkt auf die `players`-Tabelle zugreift und
  # damit die in app/models/player.rb vorhandene Serializer-Konfiguration umgeht.
  ...
  def up
    Player.find_each do |player|
      ...
      raw_value = player.read_attribute_before_type_cast("data")
  ...
  def down
    Player.find_each do |player|
      ...
      begin
        ...
        raise "Fehler beim Rückumwandeln von Player #{player.id}: #{e.message}"
      end

      Player.where(id: player.id)
            .update_all("data = '#{yaml_str}'")
      # player.update_column("data", yaml_str)
    end
  end
end
